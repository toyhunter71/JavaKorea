...
//		int cat_count = 0;
		...
		while(select != 0) {
			...
			select = Integer.parseInt(sc.nextLine());         //메뉴 선택  // 문자열 -> int
			...
			if(select == 1) {
				...
				if(type == 0) {
					//개 추가
					pet_list[pet_count++] = new Dog();     //다형성 확인 부분
					System.out.println("개를 추가 했습니다.");
				}else if(type == 1) {
					//고양이 추가
					pet_list[pet_count++] = new Cat();     //다형성 확인 부분
					System.out.println("고양이를 추가 했습니다.");
				}else if(type == 2) {
					//새 추가
					pet_list[pet_count++] = new Bird();    //다형성 확인 부분
					System.out.println("새를 추가 했습니다.");
				}
			}else if(select == 2) {
				//울기
